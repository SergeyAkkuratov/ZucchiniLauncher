...
import { taskSlice, errorSlice, useAppDispatch, useAppSelector } from "../../store/Store";
    ...
    return (
        <>
            <Navbar expand="lg" className="bg-body-tertiary bg-dark" data-bs-theme="dark" data-testid="header">
                ...
                {user.username ? (
                    <Badge bg={user.role === "[ROLE_ADMIN]" ? "success" : "primary"} onClick={() => navigate("/login?logout")}>{user.username}</Badge>
                ) : (
